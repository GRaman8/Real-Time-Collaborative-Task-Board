...
import { updateBoardSchema } from '../../validation/boardValidation';
...
export default function EditBoardModal({ isOpen, onClose, onSubmit, board }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = updateBoardSchema.safeParse(form);
    if (!result.success) {
      const fieldErrors = {};
    ...
    setLoading(true);
    try {
      await onSubmit(board._id, result.data);
      onClose();
    } catch (err) {
      setErrors({ general: err.response?.data?.msg || 'Failed to update board' });
    } finally {
      setLoading(false);
    }
  };
